# 对象分类卡片模式（strategy-flow-input/0.6）

## 定位

`classification` 是 0.3 引入、0.4 起继续使用的流程卡片类型，中文展示名为“对象分类”。它表示执行人对当前对象做分类、分层或重新分层；对象本身没有行为要求，也不因此被触达。
...
0.2 / 0.3 / 0.4 / 0.5 导入 0.6 编辑器后仍保持 `process`。业务人员显式把卡片类型改为“对象分类”后，导出才使用 `classification`。

通用完整示例见 [`../examples/contracts/strategy-flow-input-0.6.json`](../examples/contracts/strategy-flow-input-0.6.json)。
