...
Design 0.6 用 `nodes[].nodeType = "classification"` 表达“对象分类”卡片。它可作用于客群、场景、事件或活动，表示执行人对对象做分类、分层或重新分层；对象本身无行为要求，也不因此被触达。
...
- 当前离线 stub 生成 Design 0.6，但不会自动生成 `classification`。Agent 发现信号后只能在审查汇报 / openQuestions 中提示，待业务人员确认后在编辑器显式添加或修改卡片。
