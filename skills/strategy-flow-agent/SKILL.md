...
description: 使用 strategy-flow-input 工作台解析非结构化策略材料、识别 Design 0.5 对象分类 / 分层信号、生成证据支撑的策略流程草稿，并引导业务人员确认和导出双 JSON；适用于 Hermes、openclaw、Codex 等 Agent 接入场景。
---
...
4. 协助人工在编辑器中完成 Design 0.5 与 Metadata 2.0，而不是绕过工作台提交。
...
- 对象分类使用 Design 0.5 的 `classification` 卡片表达；显式分类 / 分层证据只能形成候选意图，弱信号只能作为 openQuestion，不得直接编造节点。
